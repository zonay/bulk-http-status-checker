...
const urlTreeDataProvider_1 = require("./urlTreeDataProvider");
const exportUtils_1 = require("./utils/exportUtils");
function activate(context) {
    const urlProvider = new urlTreeDataProvider_1.UrlTreeDataProvider();
    ...
    context.subscriptions.push(vscode.commands.registerCommand('urlStatusChecker.addUrl', async () => {
        try {
            ...
            if (input === 'Paste URLs from clipboard') {
                ...
                const url = await vscode.window.showInputBox({
                    ...
                    validateInput: text => text.startsWith('http://') || text.startsWith('https://')
                        ? null
    ...
    context.subscriptions.push(vscode.commands.registerCommand('urlStatusChecker.clearUrls', () => {
    ...
    let disposable = vscode.commands.registerCommand('urlStatusChecker.checkStatus', async () => {
        ...
        const results = [];
        const panel = vscode.window.createWebviewPanel('urlStatusReport', 'Bulk HTTP Status Report', vscode.ViewColumn.Two, { enableScripts: true });
        ...
        await vscode.window.withProgress({
            ...
            for (let i = 0; i < urls.length; i++) {
                ...
                try {
                    const result = await (0, statusChecker_1.getStatusChain)(currentUrl);
                    results.push(result);
                }
                catch (error) {
                    results.push({
            ...
            // Show final results
            panel.webview.html = (0, htmlGenerator_1.generateHtmlReport)(results);
        });
            ...
            if (message.command === 'export' && !exportInProgress) {
                ...
                try {
                    if (message.type === 'json') {
                        await (0, exportUtils_1.copyToJson)(results);
                    }
                    else if (message.type === 'csv') {
                        await (0, exportUtils_1.exportToCsv)(results);
                    }
